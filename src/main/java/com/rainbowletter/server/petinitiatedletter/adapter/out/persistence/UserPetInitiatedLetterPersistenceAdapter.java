...
public class UserPetInitiatedLetterPersistenceAdapter {
    ...
    public List<UserPetPairDto> findAllUserPetPairs() {
        return queryFactory.select(Projections.constructor(
            UserPetPairDto.class,
            userPetInitiatedLetter.userId,
            userPetInitiatedLetter.petId
        ))
            .from(userPetInitiatedLetter)
            .join(userJpaEntity).on(userPetInitiatedLetter.userId.eq(userJpaEntity.id))
            .where(
                userJpaEntity.petInitiatedLetterEnabled.eq(true)
            )
            .fetch();
    }
